import {
  ...
  HelpCircle, Moon, CloudMoon
} from "lucide-react";
import type { LucideIcon } from "lucide-react";

export function weatherCodeToIcon(code: number, isDay: boolean | null): LucideIcon {
  ...
  const map: Record<number, LucideIcon> = {
    ...
    61: CloudRain, 63: CloudRain, 65: CloudRain, 80: CloudRain,
...
export function weatherCodeToBg(code: number | null, isDay: boolean | null): string {
  const night = isDay === false;
  ...
  if ([3, 45, 48].includes(code)) return night ? "bg-cloudy-night" : "bg-cloudy";
  if ([51, 53, 55, 61, 63, 65, 80].includes(code)) return night ? "bg-rainy-night" : "bg-rainy";
